 ...
 */
(function() {
	function makemodxlinkDialog( editor, isEdit ) {

		var plugin = CKEDITOR.plugins.modxlink;
		...
		var linkTypeChanged = function() {
			var dialog = this.getDialog(),
				partIds = [ 'urlOptions', 'pageOptions', 'anchorOptions', 'emailOptions' ],
		...
		var parseLink = function( editor, element ) {
			var href = ( element && ( element.data( 'cke-saved-href' ) || element.getAttribute( 'href' ) ) ) || '',
				javascriptMatch, emailMatch, anchorMatch, urlMatch, pageMatch,
				retval = {};
			...
			if ( !retval.type ) {
				...
				if( ( pageMatch = href.match( modxPageRegex ) ) ) {
					retval.type = 'page';
					retval.sitePage = {};
					retval.sitePage.url = href;
					retval.sitePage.pageid = retval.sitePage.id = pageMatch[ 1 ];
				}
			...
			if ( element ) {
				...
				CKEDITOR.plugins.modxlink.synAnchorSelector ? retval.adv.advCSSClasses = getLinkClass( element ) : advAttr( 'advCSSClasses', 'class' );
		...
		var commonLang = editor.lang.common,
			linkLang = editor.lang.link, modxlinkLang = editor.lang.modxlink;
									...
									children: [
										{
											...
											setup: function( data ) {
//											console.log('On setup ',data);
												if ( data.url )
													this.setValue( data.url.protocol || '' );
											},
											commit: function( data ) {
//											console.log('Commiting data',data);
												if ( !data.url )
													data.url = {};
...
//											var directValue = dialog.getValueOf( 'info', 'protocol' );
//											var theElement = dialog.getContentElement( 'info', 'protocol' );

//											console.log("Element in dialog",theElement);

//											console.log("Value from dialog",directValue);

//console.log(this.getValue());
												data.url.protocol = this.getValue();
//											console.log('After commit',data);
											}
										},
										{
											type: 'text',
...
//console.log("URL in url keyup",url);
...
/*									this.allowOnChange = false;
								...
								validate: function() {
									...
									if ( dialog.getContentElement( 'info', 'linkType' ) && dialog.getValueOf( 'info', 'linkType' ) != 'page' )
									return true;
								...
								,setup:function( data ) {
									this.allowOnChange = false;
									if ( data.sitePage )
									this.setValue( data.sitePage.pageid );
									this.allowOnChange = true;
									},commit: function( data ) {
									// IE will not trigger the onChange event if the mouse has been used
									// to carry all the operations #4724
								...
								data.sitePage.pageid = this.getValue();
								data.sitePage.url = "[[~"+data.sitePage.pageid+"]]";
								this.allowOnChange = false;
								}
								}
			...
			switch ( data.type || 'url' ) {
				case 'url':
					var protocol = ( data.url && data.url.protocol != undefined ) ? data.url.protocol : 'http://',
								...
								case 'page':
								var protocol = '',
								pageid = ( data.sitePage.pageid && CKEDITOR.tools.trim( data.sitePage.pageid ) ) || '';
								// @todo Add options for caching, link scheme etc
								attributes[ 'data-cke-saved-href' ] = '[[~' + data.sitePage.pageid + ']]';
								break;
								case 'anchor':
			...
			if ( !this._.selectedElement ) {
				...
				if ( href == textView || data.type == 'email' && textView.indexOf( '@' ) != -1 ) {
								...
								onLoad: function() {
									...
									//console.log(CKEDITOR.UI_RICHCOMBO);
									},
								...
								CKEDITOR.dialog.add( 'makemodxlink', function( editor ) {
		return makemodxlinkDialog( editor );
	});

/*	CKEDITOR.dialog.add( 'editmodxlink', function( editor ) {
		return makemodxlinkDialog( editor, 1 );
	});*/

/*	CKEDITOR.plugins.modxlink={synAnchorSelector:function(b){var a=b.getSelection(),c=a.getSelectedElement();return c&&c.is("a")?c:(a=a.getRanges(!0)[0])?(a.shrink(CKEDITOR.SHRINK_TEXT),b.elementPath(a.getCommonAncestor()).contains("a",1)):null},fakeAnchor:CKEDITOR.env.opera||CKEDITOR.env.webkit,synAnchorSelector:CKEDITOR.env.ie,emptyAnchorFix:CKEDITOR.env.ie&&8>CKEDITOR.env.version,tryRestoreFakeAnchor:function(b,a){if(a&&a.data("cke-real-element-type")&&"anchor"==a.data("cke-real-element-type")){var c=b.restoreRealElement(a);
		if(c.data("cke-saved-name"))return c}}}

	CKEDITOR.plugins.modxlink={getSelectedLink:function(b){var a=b.getSelection(),c=a.getSelectedElement();return c&&c.is("a")?c:(a=a.getRanges(!0)[0])?(a.shrink(CKEDITOR.SHRINK_TEXT),b.elementPath(a.getCommonAncestor()).contains("a",1)):null},fakeAnchor:CKEDITOR.env.opera||CKEDITOR.env.webkit,synAnchorSelector:CKEDITOR.env.ie,emptyAnchorFix:CKEDITOR.env.ie&&8>CKEDITOR.env.version,tryRestoreFakeAnchor:function(b,a){if(a&&a.data("cke-real-element-type")&&"anchor"==a.data("cke-real-element-type")){var c=b.restoreRealElement(a);
		if(c.data("cke-saved-name"))return c}}}*/
